...
### 1.1 启动速腾16线激光传感器 (robosense16, rs16)
...
2. *启动雷达驱动命令*：
   ...
   + 修改`launch`文件：将`velodyne_lidar.dag`修改为`lidar.dag`
...
3. *验证雷达驱动是否成功*
   ...
   + 终端输入：`cyber_monitor`，查看是否存在以下三个通道：
     ...
     + `compensator/PointCloud2`：原始数据经过**运动去畸变**后矫正的点云数据，一般Apollo使用该通道作为输入。关于补偿点云的原理讲解可以参考：[知乎补充讲解](https://zhuanlan.zhihu.com/p/109379384?from_voters_page=true)。该点云的发布**依赖gnss或者定位的tf树信息**（提供运动信息），因此如果仅启动雷达时该通道没有输出属于正常现象。
...
2. *对Apollo输出通道进行检查*
   ...
   <img src="apollo各模块启动指导文档.assets/image-20220206091509583.png" alt="image-20220206091509583" style="zoom:50%;" />
...
data {
   ...
   关于文件地理坐标系的解析详见：**Apollo中的各个坐标系** ==TODO：超链接==
...
1. 关于`header.timestamp`与`measurement_time`：
   ...
   <img src="apollo各模块启动指导文档.assets/image-20220303210504836.png" alt="image-20220303210504836" style="zoom:50%;" />
...
1. *物理连线*
   ...
   分别连接工控机与can线（连接can1口），车底盘接口与can线。
...
3. *对can通讯进行测试*
   ...
   在终端输入`candump can0`，观察结果：

   + 如果出现如下现象（只有`10x`的返回值），则证明can没有成功连接，请再次检查

     <img src="apollo各模块启动指导文档.assets/image-20220409113754096.png" alt="image-20220409113754096" style="zoom:33%;" />
...
2. 将遥控器的手柄切换至**自动驾驶模式**；
   ...
   <img src="apollo各模块启动指导文档.assets/image-20220409113322877.png" alt="image-20220409113322877" style="zoom: 33%;" />
...
==// TODO: 更新一个时序没有问题的数据集，不使用激光时钟。处理完毕后放到百度网盘中==
...
==TODO：==基于RTK定位的算法原理较为简单，接受odometry
...
1. *解析dag文件*：
   ...
   + `components.config.config_file_path`：对应的参数配置文件，以gflags形式进行处理
...
5. 启动ndt定位模块
   ...
   + 启动数据集，确保`/apollo/localization/pose`通道没有输出：
     ...
     ```bash
     cyber_recorder play -f data/bag/0326_no_localization/0326_no.record.00000
     ```
   ...
   + 尽管可视化程序在名称上归属于msf，但是它在**所有定位方式中**均可以使用。使用时需要确认：
     ...
     + 可视化结果展示。此时，按c键可以在Lidar和Fusion定位中进行切换
       ...
       <img src="apollo各模块启动指导文档.assets/2022-04-06 22-00-51屏幕截图.png" style="zoom: 33%;" />
     
     + **注意**：当地图不可显示时，如上图所示，删除缓存文件：`rm -rf cyber/data/map_visual`，并将`--local_map_name`设置为`local_map`后重新启动
     
       <img src="apollo各模块启动指导文档.assets/image-20220211223458732.png" alt="image-20220211223458732" style="zoom: 33%;" />
