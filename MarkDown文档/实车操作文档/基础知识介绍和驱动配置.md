...
### 1.2 启动速腾32线激光传感器 (robosense32, rs32)
...
1. *创建/修改配置文档*：
   ...
   + 激光驱动配置文档：`modules/drivers/lidar/conf/rs32.pb.txt`。注意，这里**是否启动雷达自身时钟**与时间同步问题有关，将会在后面详细说明。

     ```yaml
     model: "RS32"         # 32线激光雷达模型 
   ...
   + 补偿点云配置文档：`modules/drivers/lidar/conf/rs32_compensator.pb.txt`
