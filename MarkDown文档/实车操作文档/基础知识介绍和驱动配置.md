...
修改配置文件 `/apollo/modules/dreamview/conf/hmi_modes/dev_kit_debug.pb.txt`:

增加`Fusion Perception`启动按钮：

```bash
cyber_modules {
  key: "Fusion Perception"
  value: {
    dag_files: "/apollo/modules/perception/production/dag/dag_streaming_perception.dag"
  }
}
```
在文件最后修改 `monitored_components `Lidar 16 为Lidar 32：
```bash
...
2. *对Apollo输出通道进行检查*
   ...
   + 启动`cyber_monitor`：查看是否存在以下两个个通道：
     ...
     ```
     /apollo/sensor/camera/front_6mm/image                   10.00
     /apollo/sensor/camera/front_6mm/image/compressed        10.00
     ```
...
2. *配置组合导航系统相关参数*
   ...
   5. **开启移动网络**：点击`模块设置 -> WIFI 设置`， 开启Internet，连接接收机 WiFi 的载体就可以使用接收机的网络进行上网，
      + WIFI名字(SSID)为`GNSS-XXXXXXX`不可更改。其中 XXXXXXX 代表你的接收器的 SN 号。SN 号位于处理主机标签上，**在多辆车同时存在的情况下一定要多加核查**，以免产生干扰。
      + 建立WiFi连接，可关闭密码，或设置密码是`12345678`，点击保存。
   ...
   8. **重启接收机**：上述操作步骤后，需要重启接收机。在`接收机配置-重启接收机`中确定
    ...
    重启接收机后，最后关闭工控机，关闭整车电源，关闭整车电源可以断开GNSS的电源。
