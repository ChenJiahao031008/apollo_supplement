...
+ `modules/calibration/data/dev_kit_pix_hook/perception_conf/recognition_conf.pb.txt`：
...
+ `modules/calibration/data/dev_kit_pix_hook/perception_conf/lidar_cnnseg_fusion_component_conf.pb.txt`:
...
module_config {
 ...
 components {
  ...
  components {
    ...
    config {
      name: "SensorFusion"
      config_file_path: "/apollo/modules/calibration/data/dev_kit_pix_hook/perception_conf/lidar_cnnseg_fusion_component_conf.pb.txt"
      readers {
...
6. 在`dreamviewer`最终效果如下(第一个为cnnseg，第二个为PointPillar)：可以看出，雷达视野**覆盖360度**，但是感知距离相对较近。
...
+ `modules/calibration/data/dev_kit_pix_hook/perception_conf/lidar_pointpillar_fusion_component_conf.pb.txt`:
...
module_config {
  ...
  components {
    ...
    config {
      name: "SensorFusion"
      config_file_path: "//apollo/modules/calibration/data/dev_kit_pix_hook/perception_conf/lidar_pointpillar_fusion_component_conf.pb.txt"
      readers {
        channel: "/perception/inner/PrefusedObjects"
      }
    }
...
4. 查看输出结果：（camera检测输出为三维目标框）
   ...
   观察到`/perception/obstacles`中有数据输出：

   + 障碍物序列为：

   <!--换一张通道为`/perception/obstacles`的截图-->
