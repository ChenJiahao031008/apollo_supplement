...
在遥控器控制模式下，开启`cyber_recorder`记录数据，并驱动车辆绕较大的0字或者8字轨迹。创建`data/bag/localization`文件夹，
该文件夹不存在则创建，如果存在且有数据则需要提前清除：
...
1. 修改地图的范围，打开`modules/localization/msf/local_pyramid_map/base_map/base_map_config.cc`,
修改第35行：
...
3. 生成地图文件(`base_map.txt`)，其中1表示冗余区域大小为1
...
5. 建立地图文件夹（如`map_test`，可以修改为自己地图名称），并生成.bin文件
...
<img src="定位模块操作实践.assets/CASE2.png" style="zoom: 50%;" />
...
+ 注意：当地图不可显示时，如上图所示，删除缓存文件：`rm -rf cyber/data/map_visual`后重新启动
