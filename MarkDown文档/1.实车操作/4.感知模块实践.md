...
1. 启动感知模块，等待待显存稳定（一般在1-2分钟左右）
 ...
 2. 待显存稳定后启动数据集
    ...
    + 显存稳定：
      ...
      <img src="4.感知模块实践.assets/显存.png" alt="显存" style="zoom:80%;" />
 ...
 3. 查看`/apollo/perception/obstacles`中是否由数据输出（cnnseg检测输出为多边形，PointPillar检测输出为矩形框）
    ...
    <img src="4.感知模块实践.assets/雷达障碍物.png" alt="雷达障碍物" style="zoom:80%;" />
    ...
    <img src="4.感知模块实践.assets/障碍物信息.png" alt="障碍物信息" style="zoom:80%;" />

 4. 在`dreamviewer`最终效果如下(第一个为cnnseg，第二个为PointPillar)：可以看出，雷达视野**覆盖360度**，但是感知距离相对较近。

    <img src="4.感知模块实践.assets/cnnseg.png" alt="cnnseg" style="zoom: 100%;" />

    <img src="4.感知模块实践.assets/pillar.png" alt="pillar" style="zoom:80%;" />
5. 障碍物
...
3. 查看输出结果：（camera检测输出为三维目标框）
   ...
   + 障碍物序列为：
     ...
     <img src="4.感知模块实践.assets/相机障碍物.png" alt="相机障碍物" style="zoom:80%;" />
 ...
 4. 在`dreamviewer`最终效果如下：可以看出，**相机的视野较远**，还能有效的检测到**小物体**，但是**视野受限**。
    ...
    <img src="4.感知模块实践.assets/融合感知.png" alt="融合感知" style="zoom:50%;" />

## 4.4. 基于图像和激光点云的融合感知模块
...
2. 待显存稳定后启动数据集：
   ...
   + 显存稳定：
     ...
     <img src="4.感知模块实践.assets/显存.png" alt="显存" style="zoom:50%;" />
...
<img src="4.感知模块实践.assets/预测模块.png" alt="预测模块" style="zoom:80%;" />

2. 观测`cyber_monitor`的`/apollo/prediction`通道是否正常工作输出

<img src="4.感知模块实践.assets/预测障碍物.png" alt="预测障碍物" style="zoom:100%;" />
