...
Status PathBoundsDecider::Process(
  ...
  auto obs=reference_line_info->path_decision()->Find("7599");
  for(auto xy_pt : obs->PerceptionBoundingBox().GetAllCorners()){
    ...
    AINFO<<"plot_obs_7599:"<<sl_point.s()<<","<<sl_point.l();
  }
  ...
  for (const auto& lane_borrow_info : lane_borrow_info_list) {
    ...
    RecordDebugInfo(regular_path_bound, absl::StrCat("regular/", path_label, "/", borrow_lane_type), reference_line_info);
...
void PathBoundsDecider::RecordDebugInfo(
  ...
  for (const PathBoundPoint& path_bound_point : path_boundaries) {
    ...
    AINFO<<"plot_min_"<<debug_name<<":"<<
            std::get<0>(path_bound_point)<<","<<
            std::get<1>(path_bound_point);
    AINFO<<"plot_max_"<<debug_name<<":"<<
            std::get<0>(path_bound_point)<<","<<
