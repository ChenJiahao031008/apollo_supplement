...
void odom_estimation(){
    ...
    std::ofstream fout(pcd_path + "/lidar_slam_pose.txt");
    ...
    while(1){
        if(!pointCloudEdgeBuf.empty() && !pointCloudSurfBuf.empty()){
            ...
            fout  << std::fixed << pointcloud_time.toSec() <<" "<<t_current.x()<<" "<<t_current.y()<<" "<<t_current.z()<< " "
                 << q_current.x()<< " "<< q_current.y()<<" "<<q_current.z()<<" "<<q_current.w()<<std::endl;
