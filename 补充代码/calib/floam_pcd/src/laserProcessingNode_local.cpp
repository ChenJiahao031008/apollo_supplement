...
void laser_processing(){
    std::ifstream fin(pcd_path + "/pcd_timestamp.txt");
    ...
    while (!fin.eof())
    {
        ...
        pcl::io::loadPCDFile(pcd_path + "/" + split_str[0]+ ".pcd", *pointcloud_in);
        double timestamp = std::atof(split_str[1].c_str());

        std::vector<std::string> split_timestamp;
        boost::split(split_timestamp, split_str[1], boost::is_any_of("."));

        while(split_timestamp[1].length() < 9){
            split_timestamp[1] += "0";
        }

        ros::Time pointcloud_time = ros::Time(std::stoi(split_timestamp[0].c_str()),std::stoi(split_timestamp[1].c_str()));
