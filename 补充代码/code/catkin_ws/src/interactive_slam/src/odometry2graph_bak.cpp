...
private:
  pcl::PointCloud<pcl::PointXYZI>::Ptr load_cloud(const std::string& filename) const {
    ...
    if(extension == ".pcd") {
      ...
      pcl::PointCloud<PointXYZIT>::Ptr cloud_read(new pcl::PointCloud<PointXYZIT>());
      pcl::io::loadPCDFile(filename, *cloud_read);
      for(const auto &p :cloud_read->points) {
        pcl::PointXYZI pt;
        pt.x = p.x;
        pt.y = p.y;
        pt.z = p.z;
        pt.data[3] = static_cast<float>(p.intensity) / 255.0;
        pt.intensity = static_cast<float>(p.intensity) / 255.0;
        // std::cout << "pt.data[0] " << pt.data[0] << std::endl;
        // std::cout << "pt.x " << pt.x << std::endl;
        // std::cout << "pt.data[1] " << pt.data[1] << std::endl;
        // std::cout << "pt.y " << pt.y << std::endl;
        // std::cout << "pt.data[2] " << pt.data[2] << std::endl;
        // std::cout << "pt.z " << pt.z << std::endl;
        // std::cout << "pt.data[3] " << pt.data[3] << std::endl;
        // if ((pt.x * pt.x + pt.y * pt.y + pt.z * pt.z) < 2500)
          cloud->push_back(pt);
      }
...
public:
  ...
  void update_extrinsics(Eigen::Isometry3d Timu_lidar) {
    if(keyframes.empty()) {
      return;
    }
...
private:
  ...
  void load_apollo(guik::ProgressInterface& progress, const std::string& directory) {
    ...
    std::ifstream ifs(directory + "/corrected_poses.txt");
    if(!ifs) {
      return;
    }
    ...
    while(!ifs.eof()) {
      ...
      sst >> id >> time >> trans.x() >> trans.y() >> trans.z() >> quat.x() >> quat.y() >> quat.z() >> quat.w();
      static Eigen::Vector3d trans_init = Eigen::Vector3d(trans.x(), trans.y(), trans.z());

      Eigen::Isometry3d pose = Eigen::Isometry3d::Identity();
      pose.translation() = trans - trans_init;
      pose.linear() = quat.toRotationMatrix();
  ...
  virtual void draw_ui() override {
...

    {
      ...
      if(delta_updated && !updated) {
        Eigen::Isometry3d Timu_lidar_raw = Eigen::Isometry3d::Identity();
        // Timu_lidar_raw.translation() = Eigen::Vector3d(0.0, 0.0, 0.226);
        Timu_lidar_raw.translation() = Eigen::Vector3d(0.0, 0.853, -0.02);
        Timu_lidar_raw.linear() = Eigen::Quaterniond(0.707106, 0.0, 0.0, 0.707106).toRotationMatrix();

        Eigen::Isometry3d Timu_lidar_delta = Eigen::Isometry3d::Identity();
        Timu_lidar_delta.translation() = Eigen::Vector3d(x, y, z);
        Timu_lidar_delta.linear() =  Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ()) *
        ...
        odometry_set->update_extrinsics(Timu_lidar_delta);

        Eigen::Isometry3d Timu_lidar = Timu_lidar_raw * Timu_lidar_delta;
